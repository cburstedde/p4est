                          ...
                          p4est_quadrant_t * quad, p4est_init_t init_fn)
{
  ...
  if (init_fn != NULL && p4est_quadrant_is_valid (quad)) {
      ...
      for (l = 0; l < 3; ++l) {
        ...
        *s = *inq;
        ...
        if (p4est_quadrant_compare (&fd, &treefd) <= 0) {
          ...
          first_index = p4est_find_lower_bound (&tree->quadrants, s, guess);
          if (first_index < 0) {
            continue;
          }
          guess = first_index;
                           ...
                           int32_t which_tree, p4est_init_t init_fn)
{
  ...
  last_valid = incount - 1;
  p4est_quadrant_last_descendent (q, &tree_last, inmaxl);
  for (i = first_valid + 1; i < incount; ++i) {
    q = p4est_array_index (inlist, i);
    if (!p4est_quadrant_is_valid (q)) {
      last_valid = i - 1;
      break;
  ...
  P4EST_ASSERT (first_valid <= last_valid && last_valid < incount);
  ...
  for (l = inmaxl; l > 0; --l) {
    ...
    for (i = 0; i < incount + ocount; ++i) {
      ...
      for (sid = 0; sid < bbound; ++sid) {
        ...
        if (p4est_quadrant_is_valid (qalloc)) {
                       ...
                       const int32_t * new_num_quadrants_in_proc)
{
  ...
  p4est->global_num_quadrants = new_global_last_quad_index[num_procs - 1] + 1;
