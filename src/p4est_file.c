  ...
  *connectivity = NULL;
  ...
  while (fgets (buf, BUFSIZ, file)) {
    ...
    if (*line == '\0') {
      ...
      P4EST_CHECK_ABORT(line[length-1] == ']', "Sections must end with ']'");
      ...
      P4EST_CHECK_ABORT(
          section==INFO || *connectivity!=NULL,
                                              ...
                                              *connectivity)
{
  ...
  for(k=0; k<num_trees; ++k)
    printf("    %d    %d    %d    %d    %d\n",
        k+1, tree_to_vertex[4*k+0]+1, tree_to_vertex[4*k+1]+1,
        tree_to_vertex[4*k+2]+1, tree_to_vertex[4*k+3]+1);
