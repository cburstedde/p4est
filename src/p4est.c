           ...
           size_t data_size, p4est_init_t init_fn)
{
  ...
  for (jl = 0; jl < num_trees; ++jl) {
    tree = p4est_array_index (p4est->trees, jl);
  ...
  for (jl = first_tree; jl <= last_tree; ++jl) {
    ...
    P4EST_DEBUGF ("tree %lld first morton 0x%llx 0x%llx\n",
    ...
    if (tree_num_quadrants == 1 ||
      ...
      *quad = a;
      ...
      P4EST_DEBUGF ("tree %lld last morton 0x%llx 0x%llx\n",
...
p4est_copy (p4est_t * input, int copy_data)
{
  ...
  p4est_array_t      *iquadrants, *pquadrants;
  ...
  for (j = 0; j < num_trees; ++j) {
    ...
    p4est_array_init (&ptree->quadrants, sizeof (p4est_quadrant_t));
  }
  for (j = first_tree; j <= last_tree; ++j) {
    ...
    p4est_array_resize (pquadrants, icount);
    ...
    while (list->elem_count > 0) {
      ...
      if (dorefine ||
        ...
        if (restpos < incount && current == restpos) {
          ...
          while (movecount > 0) {
            q = p4est_array_index (tquadrants, restpos);
            qalloc = p4est_mempool_alloc (p4est->quadrant_pool);
               ...
               p4est_init_t init_fn)
{
  ...
  for (j = p4est->first_local_tree; j <= p4est->last_local_tree; ++j) {
    ...
    while (rest + 3 - before < incount) {
      ...
      for (k = 0; k < 4; ++k) {
        if (k < before) {
          c[k] = p4est_array_index (tquadrants, first + k);
          ...
          c[k] = p4est_array_index (tquadrants, rest + k - before);
        }
      }
      if (couldbegood &&
        ...
        if (first > last) {
          if (first != rest) {
            cfirst = p4est_array_index (tquadrants, first);
            clast = p4est_array_index (tquadrants, rest);
            *cfirst = *clast;
          }
          last = first;
          ++rest;
    ...
    if (first + 1 < rest) {
      ...
      p4est_array_resize (tquadrants, first + 1);
    }
                        ...
                        p4est_balance_peer_t * peer)
{
  ...
  P4EST_DEBUGF ("first load from %d into %d trees\n", peer_id,
                num_receive_trees);
...
p4est_balance (p4est_t * p4est, p4est_init_t init_fn)
{
  ...
  p4est_array_resize (peers, num_procs);
  for (i = 0; i < num_procs; ++i) {
  ...
  for (i = 0; i < nwin; ++i) {
    P4EST_DEBUGF ("empty range %d from %d to %d\n",
  ...
  for (i = 0; i < nwin; ++i) {
    P4EST_DEBUGF ("window %d from %d to %d\n",
                  i, peer_windows[2 * i], peer_windows[2 * i + 1]);
  }
#endif /* P4EST_HAVE_DEBUG */
  ...
  for (j = first_peer; j <= last_peer; ++j) {
    ...
    if (qcount > 0) {
      P4EST_DEBUGF ("Balance A send %d quadrants to %d\n", qcount, j);
      ++send_load[0];
    }
    else {
      ++send_zero[0];
    ...
    if (qcount > 0) {
      ...
      P4EST_DEBUGF ("Balance A send checksum %x to %d\n", checksum, j);
#endif /* P4EST_HAVE_DEBUG */
  ...
  while (request_first_count > 0) {
    ...
    for (i = 0; i < outcount; ++i) {
      ...
      if (!peer->have_first_count) {
        ...
        P4EST_CHECK_MPI (mpiret);
        P4EST_CHECK_ABORTF (rcount == 1,
                            "Receive count mismatch A %d", rcount);
        ...
        P4EST_DEBUGF ("Balance A recv checksum %x from %d\n", checksum, j);
#endif /* P4EST_HAVE_DEBUG */
        ...
        if (qcount > 0) {
          P4EST_DEBUGF ("Balance B send %d quadrants to %d\n", qcount, j);
          ++send_load[1];
        }
        else {
          ++send_zero[1];
        ...
        if (qcount > 0) {
          ...
          P4EST_DEBUGF ("Balance B send checksum %x to %d\n", checksum, j);
#endif /* P4EST_HAVE_DEBUG */
  ...
  while (request_second_count > 0) {
    ...
    for (i = 0; i < outcount; ++i) {
      ...
      if (!peer->have_second_count) {
        ...
        P4EST_CHECK_MPI (mpiret);
        P4EST_CHECK_ABORTF (rcount == 1,
                            "Receive count mismatch B %d", rcount);
  ...
  for (j = first_tree; j <= last_tree; ++j) {
    ...
    if (first_index > 0) {
      ...
      while (first_index + k <= last_index) {
        q = p4est_array_index (tquadrants, k);
        s = p4est_array_index (tquadrants, first_index + k);
        if (k < first_index) {
    ...
    p4est_array_resize (tquadrants, qcount);
...
p4est_partition (p4est_t * p4est, p4est_weight_t weight_fn)
{
  ...
  if (weight_fn == NULL) {
     ...
     */
    P4EST_DEBUGF ("send bounds low %d high %d\n", send_lowest, send_highest);
    ...
    if (my_lowcut == 0) {
      ...
      for (; i < num_procs; ++i) {
        if (global_weight_sums[i] < my_lowcut &&
          ...
          P4EST_DEBUGF ("receive low cut from %d\n", i);
    ...
    if (my_highcut == 0) {
      ...
      for (; i < num_procs; ++i) {
        if (global_weight_sums[i] < my_highcut &&
          ...
          P4EST_DEBUGF ("receive high cut from %d\n", i);
    ...
    P4EST_DEBUGF ("my cut low %lld high %lld\n",
    ...
    P4EST_CHECK_MPI (mpiret);
    if (my_lowcut != 0) {
      P4EST_CHECK_ABORT (recv_statuses[0].MPI_SOURCE == low_source,
                         "Wait low source");
      P4EST_CHECK_ABORT (recv_statuses[0].MPI_TAG ==
                         P4EST_COMM_PARTITION_WEIGHTED_LOW, "Wait low tag");
      mpiret = MPI_Get_count (&recv_statuses[0], MPI_LONG_LONG, &rcount);
      P4EST_CHECK_MPI (mpiret);
      P4EST_CHECK_ABORTF (rcount == 1, "Wait low count %d", rcount);
    }
    if (my_highcut != 0) {
      P4EST_CHECK_ABORT (recv_statuses[1].MPI_SOURCE == high_source,
                         "Wait high source");
      P4EST_CHECK_ABORT (recv_statuses[1].MPI_TAG ==
                         P4EST_COMM_PARTITION_WEIGHTED_HIGH, "Wait high tag");
      mpiret = MPI_Get_count (&recv_statuses[1], MPI_LONG_LONG, &rcount);
      P4EST_CHECK_MPI (mpiret);
      P4EST_CHECK_ABORTF (rcount == 1, "Wait high count %d", rcount);
    }

    /* communicate the quadrant ranges */
    qcount = recv_high - recv_low;
    P4EST_ASSERT (qcount >= 0);
    P4EST_DEBUGF ("weighted partition count %d\n", qcount);
