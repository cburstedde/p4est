               ...
               size_t data_size, p4est_init_t init_fn, void *user_pointer)
{
  ...
  for (jt = first_tree; jt <= last_tree; ++jt) {
    ...
    if (!fill_uniform) {        /* fill with coarsest possible quadrants */
      ...
      if (jt == first_tree) {
        p4est_lid_set_uint64 (&temp, first_tree_quadrant);
        p4est_quadrant_set_morton_ext128 (&a, level, &temp);
      }
      else {
        p4est_lid_set_zero (&temp);
        p4est_quadrant_set_morton_ext128 (&a, level, &temp);
      }
#ifdef P4_TO_P8
      ...
      if (tree_num_quadrants == 1 ||
        ...
        *quad = a;
        ...
        if (jt == last_tree) {
          ...
          p4est_lid_set_uint64 (&temp, quadrant_index);
          p4est_quadrant_set_morton_ext128 (&b, level, &temp);
        }
        else {
          p4est_lid_set_uint64 (&temp, tree_num_quadrants - 1);
          p4est_quadrant_set_morton_ext128 (&b, level, &temp);
        }
#ifdef P4_TO_P8
      ...
      for (miu = 0; miu < count; ++miu) {
        ...
        p4est_lid_set_uint64 (&temp, first_morton + miu);
