  ...
  for (p = 0; p < mpisize; p++) {
    ...
    for (zm = (size_t) first_mirror; zm < (size_t) end_mirror; zm++) {
      ...
      for (e = 0; e < P8EST_EDGES; e++) {
        ...
        /* get the neighbors */
        p8est_quadrant_edge_neighbor_extra (mq, t, e, tempquads2, temptrees2,
                                            npoints, conn);
      ...
      for (c = 0; c < P4EST_CHILDREN; c++) {
        ...
        for (zz = 0; zz < tempquads2->elem_count; zz++) {
          ...
          /* add any quadrant that overlaps this neighbor and touches the mirror
           * to the buffer */
          p4est_ghost_expand_int (t, mq, nt, nq, &pview, P4EST_CONNECT_CORNER,
                                  nc, tempquads, temptrees, p, p4est, ghost,
                                  send_bufs);
        }
        sc_array_reset (tempquads2);
        sc_array_reset (temptrees2);
...
void
p4est_ghost_support_nodes (p4est_t * p4est, p4est_ghost_t * ghost)
{
#ifdef P4EST_MPI
  ...
  P4EST_GLOBAL_PRODUCTIONF ("Into " P4EST_STRING "_ghost_support_nodes %s\n",
  ...
  ctype = p4est_connect_type_int (ghost->btype);
  ...
  /* update mirror structures */
  {
    ...
    for (p = 0; p < mpisize; p++) {
      ...
      for (il = 0; il < old_count; il++) {
        ...
        q =
           p4est_quadrant_array_index (mirrors, (size_t)
  ...
  /* update the ghost layer */
  {
    ...
    for (i = 0; i < n_comm; i++) {
      ...
      P4EST_LDEBUGF ("ghost layer support nodes receiving  %lld new from %d\n",
    ...
    if (new_ghosts->elem_count > ghosts->elem_count) {
      ...
      P4EST_ASSERT (sc_array_is_sorted (new_ghosts, p4est_quadrant_compare_piggy));
#ifdef P4EST_DEBUG
      {
        size_t zz;
        ...
        for (zz = 0; zz < ghosts->elem_count - 1; zz++) {
          ...
          p4est_quadrant_t * q2 = p4est_quadrant_array_index (ghosts, zz+1);
        ...
        for (zz = 0; zz < new_ghosts->elem_count - 1; zz++) {
          p4est_quadrant_t * q1 = p4est_quadrant_array_index (new_ghosts, zz);
          p4est_quadrant_t * q2 = p4est_quadrant_array_index (new_ghosts, zz+1);
  ...
  p4est_log_indent_pop ();
  P4EST_GLOBAL_PRODUCTION ("Done " P4EST_STRING "_ghost_support_nodes\n");
#endif
}
