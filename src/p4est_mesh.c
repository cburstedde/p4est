                                    ...
                                    p4est_locidx_t * nedgef, int cz, int zz)
{
  ...
  for (z2 = 0; z2 < cz; ++z2) {
    ...
    ignore = 0;
    P4EST_ASSERT (0 <= z2 && z2 < info->sides.elem_count);
    ...
    if (info->tree_boundary == P4EST_CONNECT_FACE) {
      ...
      /* only inter-tree edges are processed here */
      SC_ABORT_NOT_REACHED ();
    }
    if (!ignore) {
                 ...
                 int *n_entity)
{
  ...
  P4EST_ASSERT (l_double_size <= enc);

  if (l_double_size <= enc && enc < u_double_size) {
    int                 e = enc;
    e -= l_double_size;
