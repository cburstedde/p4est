...
static const int p4est_hanging_skip[4][2] = {
  ...
  { 1, 2 }};
#else
static const int p8est_corner_edge_faces[8][12] = {
                           ...
                           const p4est_quadrant_t * q)
{
  ...
  contacts =
    ((quad_contact[0] || quad_contact[1]) ? 1 : 0) +
    ((quad_contact[2] || quad_contact[3]) ? 1 : 0) +
    ((quad_contact[4] || quad_contact[5]) ? 1 : 0);

  /* Make sure we are not a tree corner */
  P4EST_ASSERT (contacts == 1); /* edges not implemented yet */
#endif
                                        ...
                                        sc_array_t * q_procs, bool * nurgood)
{
  ...
  for (ctree = 0; ctree < cta->elem_count; ++ctree) {
    ...
    *proc = p4est_comm_find_owner (p4est, ct->ntree, &cq, rank);
    ...
    if (nurgood != NULL) {
      p4est_quadrant_last_descendent (&cq, &cq, P4EST_MAXLEVEL);
...
#else
    P4EST_ASSERT (face == -1 && edge >= 0 && corner == -1);
    if (edge >= 0) {
                                 ...
                                 int face, const p4est_quadrant_t * q)
{
  ...
  on_boundary = (tree_boundary && (xyz == ((face & 0x01) ? (rh - qh) : 0)));
#endif

  return on_boundary;
}

/** Get the same-size corner neighbor of \a q.
 *
 ...
 * \param [out] n0     Filled with the same-size corner neighbor.
 * \param [out] n0ur   If not NULL, it is filled with smallest quadrant
 *                     that fits in the upper right corner of \a n0.
 */
static void
p4est_quadrant_get_full_corner_neighbor (const p4est_quadrant_t * q,
                                         int corner,
                                         p4est_quadrant_t * n0,
                                         p4est_quadrant_t * n0ur)
{
  ...
  n0->x = q->x + (2 * (corner & 0x01) - 1) * qh;
  n0->y = q->y + ((corner & 0x02) - 1) * qh;
#ifdef P4_TO_P8
  n0->z = q->z + ((corner & 0x04) / 2 - 1) * qh;
#endif
  n0->level = q->level;
                                   ...
                                   int corner, const p4est_quadrant_t * q)
{
  ...
  p4est_quadrant_get_full_corner_neighbor (q, corner, &q2, NULL);

#ifndef P4_TO_P8
  if (q2.y < 0) {
    if (conn->tree_to_tree[2 * P4EST_DIM * treeid + 0] == treeid &&
        (int) conn->tree_to_face[2 * P4EST_DIM * treeid + 0] == 0) {
      on_boundary = true;
    }
  }
  else if (q2.x >= rh) {
    if (conn->tree_to_tree[2 * P4EST_DIM * treeid + 1] == treeid &&
        (int) conn->tree_to_face[2 * P4EST_DIM * treeid + 1] == 1) {
      on_boundary = true;
    }
  }
  else if (q2.y >= rh) {
    if (conn->tree_to_tree[2 * P4EST_DIM * treeid + 2] == treeid &&
        (int) conn->tree_to_face[2 * P4EST_DIM * treeid + 2] == 2) {
      on_boundary = true;
    }
  }
  else if (q2.x < 0) {
    if (conn->tree_to_tree[2 * P4EST_DIM * treeid + 3] == treeid &&
        (int) conn->tree_to_face[2 * P4EST_DIM * treeid + 3] == 3) {
      on_boundary = true;
    }
  }
#else
  /* TODO: figure out both edge and face boundary situation */
#endif
...
#ifdef P4_TO_P8
  sc_array_init (&e0b_a, sizeof (int));
#endif
  ...
  for (nt = first_local_tree; nt <= last_local_tree; ++nt) {
    ...
    for (li = 0; li < num_quadrants; ++li) {
      ...
      for (edge = 0; edge < 12; ++edge) {
        ...
        e0b = p4est_quadrant_exists (p4est, &ghost_layer, nt, &n[1], &e0b_a);
        if (e0 != e0b || e0_a.elem_count != e0b_a.elem_count) {
          P4EST_NOTICE ("Contradicting small edge neighbors\n");
          failed = true;
          goto failtest;
        }
        e1 = p4est_quadrant_exists (p4est, &ghost_layer, nt, &n[2], &e1_a);
        e2 = p4est_quadrant_exists (p4est, &ghost_layer, nt, &n[3], &e2_a);
        ...
        face = p8est_corner_edge_faces[qcid][edge];
        if (face >= 0 && bigger_face[face]) {
          if (e1_a.elem_count > 0 || e0 || e1 || e2) {
            P4EST_NOTICE ("Duplicate edges across hanging face\n");
            failed = true;
            goto failtest;
          }
        }
          ...
          if (e1_a.elem_count == 0 && (int) e0 + (int) e1 + (int) e2 != 1) {
          ...
          for (cez = 0; cez < e1_a.elem_count; ++cez) {
            ...
            pe0b =
              (e0b_a.elem_count > 0) ? sc_array_index (&e0b_a, cez) : &zero;
            pe1 = sc_array_index (&e1_a, cez);
            pe2 = (e2_a.elem_count > 0) ? sc_array_index (&e2_a, cez) : &zero;
            if (*pe0 + *pe1 + *pe2 != 1 || *pe0 != *pe0b) {
              P4EST_NOTICE ("Star edge balance violated\n");
              failed = true;
              goto failtest;
            }
          }
...
#endif
        if (q->level == P4EST_MAXLEVEL) {
          p4est_quadrant_get_full_corner_neighbor (q, zcorner, &n[0], NULL);
          maxed = true;
        }
        else {
