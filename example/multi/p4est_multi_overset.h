...
/** Return true if \a p4est is an artifical p4est.
 *
 * This function can be used in \ref p4est_intersect_t callbacks to
 * distinguish temporary, artificial p4est instances that were created for a
 * local partition search (and only contain some meta information) from a real
 * p4est used for a local search.
 */
int p4est_is_meta (p4est_t *p4est);

/** Callback function for checking if a point intersects a quadrant.
 *
 * This function can be passed to \ref p4est_multi_overset to search for all
 * pairs of a quadrant from the background p4est and a query point from a
 * overset mesh, which is contained in the quadrant.
 * It will be called both in a \ref p4est_search_partition and a
 * \ref p4est_search_local.
 * Use \ref p4est_is_meta, to determine which is the case.
 ...
 *                              into local quadrant storage of \a p4est.
 ...
 *                              search (p4est_is_meta and the mpirank of
 *                              \a p4est is non-negative.) this callback should
 ...
 * Functions of this type can be passed to \ref p4est_multi_overset to compute
 * interpolation data for query points based on the background p4est.
 * \param [in] p4est            A valid p4est.
 ...
 *                              to \ref p4est_multi_overset, the interpolation
 *                              values should be computed based on \a point and
 *                              the data stored in \a quadrant and \a p4est.
 */
typedef void (*p4est_interpolate_point_t) (p4est_t *p4est,
                                           p4est_topidx_t which_tree,
                                           p4est_quadrant_t *quadrant,
                                           p4est_locidx_t local_num,
                                           void *point,
 ...
 *
 * Exchange interpolation data between a p4est background mesh \a bgp4est and
 ...
 *    \a bgp4est it is located in and the quadrant does not contain another
 ...
 *                              \a bgp4est (see also \ref p4est_intersect_t).
 ...
 *                              local search in \a bgp4est and during a
 *                              partition search in \a bgp4est on a overset
 ...
 *                              (see also \ref p4est_interpolate_point_t).
 *                              This function receives a leaf quadrant of
 *                              \a bgp4est and a point in \a qpoints for which
 ...
 *                              actual \a bgp4est and the corresponding quadrant.
...
void                 p4est_multi_overset
  (sc_MPI_Comm glocomm, sc_MPI_Comm headcomm, sc_MPI_Comm rolecomm,
   ...
   p4est_t *bgp4est, sc_array_t *qpoints, p4est_intersect_t intsc_fn,
