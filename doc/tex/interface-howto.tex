...
\caption{Part B, changing the refinement structure and partition.  With the
  ...
  leaves, which will be outdated and destroyed once \texttt{p} is changed
  again.  Refinement and coarsening are controlled by callback functions that
  usually query flags
...
\caption{Part C, creating an application-specific numbering of degrees of freedom.
  ...
  then be used to identify node locations and their hanging status.  The \texttt{C}
