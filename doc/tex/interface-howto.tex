...
\node (createconn) {\texttt{c =} (choose one of the following)}
child  {
...
\node[object] at (12.5, 0) (theconn) { \texttt{c} };
...
\node[object] at (0, -4.5-.75) (predef) { predefined };
...
\caption{Part A, creating the coarse mesh connectivity.  The connectivity
  ...
  file formats (green box) or by using one of several \pforest convenience
  functions.  The data format is documented in the big comment blocks in
...
\node (p4estnew) { \parbox{6.0cm}{\texttt{p = p4est\_new (c)}\\
                                (modify \texttt{p} in place in the following)} }
  child {
...
\node[object, xshift=11cm] (output) {\texttt{p}, \texttt{g}};
\draw[arrdat] (p4estnew) -- (output);
\draw[arrdat] (ghostnew.east) -- (output.south);

\end{tikzpicture}
\end{center}
\caption{Part B, changing the refinement structure and partition.}
\label{fig:partB}
\end{figure}
