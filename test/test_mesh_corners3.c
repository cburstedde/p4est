...
int main(int argc, char **argv)
{
    ...
    p8est_connectivity_t* conn = p8est_connectivity_new_brick(2, 2, 1, 0, 0, 0);
    p8est_t* p8est = p8est_new_ext(mpicomm, conn, 0, 1, 0, 0, NULL, NULL);

    // refine all quadrents to level 2 except quadrent in tree 3 touching 0,0,0
    ...
    // assuming that the coarse quadrent is zero in tree 3 with ordering
    p4est_locidx_t coarse_qid = 192;

    // upper and lower x,y,z indices
    p4est_locidx_t lx_idx = P8EST_FACES * coarse_qid + 0;
    p4est_locidx_t ux_idx = P8EST_FACES * coarse_qid + 1;
    p4est_locidx_t ly_idx = P8EST_FACES * coarse_qid + 2;
