...
main (int argc, char **argv)
{
  ...
  P4EST_GLOBAL_INFOF ("%s: Into test 1\n", this_fn_name);
  {
    ...
    if (is_nonempty) {
      ...
      p4est_vtk_write_file (p4est_subcomm, NULL, P4EST_STRING "_subcomm_sub1");
  ...
  P4EST_GLOBAL_INFOF ("%s: Into test 3\n", this_fn_name);
  {
    ...
    reserve_range[0][1] = 0;
    reserve_range[0][2] = 1;
    mpiret = sc_MPI_Group_range_incl (group, 1, reserve_range, &group_reserve);
    SC_CHECK_MPI (mpiret);
